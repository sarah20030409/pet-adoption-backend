...
# increasing parent_id
    def save_comment_data(oId:int,pId:int,comment:str,parent_id: int = None):
        if parent_id is None:
            query = 'INSERT INTO `Comments`(`oId`,`pId`,`comments`) VALUES(%s,%s,%s)'
            params = (oId,pId,comment)
        else:
            query = 'INSERT INTO `Comments`(`oId`,`pId`,`comments`,`parent_id`) VALUES(%s,%s,%s,%s)'
            params = (oId,pId,comment,parent_id)
    ...
    def get_comment_data(pId:int):
        query = 'SELECT `Owner`.`name` , `Comments`.`comments`, `Comments`.`CommentId`, `Comments`.`created_at`, `Comments`.`parent_id` FROM `Owner` JOIN `Comments` ON `Owner`.`oId` = `Comments`.`oId` WHERE `Comments`.`pId` = %s'
        params = (pId,)
